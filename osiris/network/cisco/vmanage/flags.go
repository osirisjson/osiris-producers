...
// reuses the vendor-agnostic helpers from cisco/run (ParseHostPort,
// PromptPassword, and LoadCredentialFile/CredentialFile for
// --secrets-file's on-disk shape and safety checks).
//
...
// instead read from --secrets-file, or, when --secrets-file not supply
...
// the same fallback chain (flag, then --secrets-file, then interactive
...
func registerFlags(fs *flag.FlagSet) *vmanageFlagValues {
	...
	fs.StringVar(&v.host, "h", "", "vManage controller host (FQDN or IP, e.g. vmanage.example.com), optionally with :port; prompted for interactively when omitted")
...
// -h/-u flag, then --secrets-file, then (for whichever of the three is
...
func ParseFlags(args []string, promptVisible, promptHidden func(string) (string, error)) (*Config, error) {
	...
	host, username, secretsFile := v.host, v.username, v.secretsFile
	port := v.port
	...
	// file has no single host of its own, see run.CredentialFile's
	// doc comment).
	// run.LoadCredentialFile (shared with apic/iosxe/nxos's
	// --secrets-file) enforces that the file is a regular, non-symlink
	// file with no group/other permission bits and where the platform
	// exposes ownership, owned by the current user.
	// --secrets-file previously loaded via a local, unchecked
	// os.ReadFile + json.Unmarshal pair with none of those guarantees.
	var tf *run.CredentialFile
	if secretsFile != "" {
		loaded, err := run.LoadCredentialFile(secretsFile)
		if err != nil {
	...
	// Still-missing host is asked for interactively (see tty.go's
	// promptVisible) rather than failing outright, so a bare
	// "osirisjson-producer cisco vmanage" works end to end. No
	// controlling terminal (e.g. cron/CI with neither the flag nor
	// --secrets-file set) is a hard error, same as before this fallback
	// chain existed.
	if host == "" {
		if promptVisible == nil {
			return nil, fmt.Errorf("--host is required (e.g. --host vmanage.example.com)")
		}
		v, err := promptVisible("vManage controller host (FQDN or IP, e.g. vmanage.example.com): ")
		if err != nil {
	...
	// rule's CIDR/exact-host match works the same way it does for
	// apic/iosxe/nxos see run.CredentialFile.ResolveForHost. vmanage
	// only ever has one target host per run, so a rules file is mostly
	// useful here for reusing one file across multiple controllers
	// (matched by host/CIDR) rather than for a per-target batch, but
	...
	// Interactive password prompt when neither -h/-u are provided
	// --secrets-file supplied one.
	password := tfPassword
