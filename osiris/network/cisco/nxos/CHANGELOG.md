...
All notable behavioral changes to the **`osirisjson-producer-cisco`** producer's NX-OS backend are documented in this file.
...
Initial Cisco NX-OS producer release. The `generatorVersion` constant has
...
### Fixed
- Wired NX-OS producer factory into the CLI dispatcher - was returning `not yet implemented` despite full implementation being present since the initial release.
...
- Resource types renamed to align with [OSIRIS JSON spec taxonomy](https://github.com/osirisjson/osiris/blob/main/specification/v1.0/OSIRIS-JSON-v1.0.md#7-resource-type-taxonomy):
  `osiris.cisco.switch.spine`, `osiris.cisco.switch.leaf`, `osiris.cisco.interface.lag`.
- Connection type `network.link` renamed to `physical.ethernet`.
- Output filename convention changed to `cisco-nxos-<timestamp>-<hostname>.json`.
