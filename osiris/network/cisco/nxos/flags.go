// flags.go - CLI flag parsing for the Cisco NX-OS OSIRIS JSON
// producer.
//
...
// registerFlags binds every NX-OS CLI flag onto fs and returns their
// destinations.
func registerFlags(fs *flag.FlagSet) *nxosFlagValues {
	v := &nxosFlagValues{}
	...
	fs.BoolVar(&v.includeRawBody, "include-raw-body", false, "attach each collected NX-API command's full, unmodified response body under extensions[\"osiris.cisco\"] on the device resource (requires --purpose audit; a lossless fallback for fields not yet modeled by this producer)")
...
// shared run.ParseCSV.
//
// promptVisible and promptHidden, when non-nil, interactively ask for
// host/username and password (respectively) on the controlling
// terminal when they are still missing after flags and --secrets-file
// have been applied. Either may be nil (e.g. no controlling terminal,
...
func ParseFlags(args []string, promptVisible, promptHidden func(string) (string, error)) (*Config, error) {
	...
	if v.source != "" {
		...
		targets, err := run.ParseCSV(v.source, "nxos")
		if err != nil {
	...
	if v.host == "" {
		...
		hv, err := promptVisible("Target host for nxos (IP or FQDN, optionally with :port): ")
		if err != nil {
...
// passwordEnvVar, then an interactive hidden prompt (promptHidden may
// be nil, e.g. no controlling terminal or batch mode, which never
// prompts). Returns "", nil if none of the three yields a value.
func resolvePassword(credPassword string, promptHidden func(string) (string, error)) (string, error) {
	...
	if promptHidden != nil {
		return promptHidden("Password for nxos authentication: ")
	}
	return "", nil
}
