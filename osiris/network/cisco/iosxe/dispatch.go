...
// Cisco IOS-XE OSIRIS JSON producer.
//
...
// Run is the CLI entry point for the IOS-XE producer. It receives the
// arguments after "iosxe".
...
// (cisco-iosxe-template.csv) and both --secrets-file shapes
// (cisco-iosxe-secrets.json, cisco-iosxe-secrets-multihost.json) via
// the shared run.GenerateTemplates.
func runTemplate(args []string) error {
	if len(args) == 0 || (args[0] != "--generate" && args[0] != "-g") {
		fmt.Println("Usage: osirisjson-producer cisco iosxe template --generate")
		return nil
	}
	return run.GenerateTemplates("iosxe", templateExampleHost)
}

// runSingle executes a single-target collection and writes it to a
// local file: cisco-iosxe-<timestamp>-<hostname>.json.
func runSingle(cfg *Config) error {
	...
	filename := fmt.Sprintf("cisco-iosxe-%s-%s.json", cfg.Timestamp, name)
...
func printHelp() {
	fmt.Print(`osirisjson-producer cisco iosxe - Cisco IOS-XE device inventory OSIRIS JSON producer
...
"iosxe template --generate" (see below) so you never have to guess the
JSON by hand:
  cisco-iosxe-secrets.json           a single login: {"host", "username", "password"}
  cisco-iosxe-secrets-multihost.json different logins per target in a batch,
...
directory structure: <output>/Datacenter/Floor/Room/Rack/Hostname.json.
  ...
  <name> template --generate	Write cisco-iosxe-template.csv and both --secrets-file skeletons

Output:
  single mode saves to: cisco-iosxe-<timestamp>-<hostname>.json (0600 permissions)
  batch mode saves to:  <output>/Datacenter/Floor/Room/Rack/Hostname.json (0600 permissions)

Examples:
  osirisjson-producer cisco iosxe -h 192.0.2.1 -u username
  osirisjson-producer cisco iosxe -h 192.0.2.1 -u username --purpose audit
  osirisjson-producer cisco iosxe -s datacenter.csv -o ./output -u username
  osirisjson-producer cisco iosxe template --generate
`)
}
