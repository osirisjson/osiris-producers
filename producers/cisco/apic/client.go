...
func NewClient(target shared.TargetConfig, insecure bool, logger *slog.Logger) *Client {
	addr := shared.ResolveAddr(target, defaultAPICPort)
	return &Client{
		baseURL:    "https://" + addr,
		httpClient: shared.NewHTTPClient(insecure),
		logger:     logger,
	}
}
