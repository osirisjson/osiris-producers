...
func newTestProducer(t *testing.T, ts *httptest.Server, detailLevel string) (*Producer, *sdk.Context) {
	...
	return &Producer{
		target: shared.TargetConfig{Host: "test", Username: "admin", Password: "test"},
		cfg:    &shared.RunConfig{DetailLevel: detailLevel},
		client: &Client{
			baseURL:    ts.URL,
...
func TestNewFactory(t *testing.T) {
	factory := NewFactory()
	p := factory(shared.TargetConfig{Host: "10.0.0.1"}, &shared.RunConfig{})
